...
- Model IDs: image = `gemini-3.1-flash-image` (NB2, only model with 512px) / `gemini-3-pro-image` (NB Pro, high-fidelity text-in-image); music = `lyria-3-pro-preview` / `lyria-3-clip-preview`; TTS = `gemini-3.1-flash-tts-preview`; research = `gemini-3.5-flash` + `deep-research-*` agents.
